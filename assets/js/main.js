...
import {
  ...
  signInWithPopup,
  onAuthStateChanged,
} from "https://www.gstatic.com/firebasejs/9.17.1/firebase-auth.js"; // add the auth
...
function already_signed_in() {
  // run at start but later
...
function send_feature() {
  ...
  if (
    user != null &&
    saved_input !== "" &&
    !saved_input.includes("<script>") &&
    !saved_input.includes("<script") &&
    !saved_input.includes("</script>")
  ) {
    // if user is signed in and message is not empty
    const dataToSet = {
    ...
    firebaseFirestore
      .addDoc(what_collection, dataToSet)
      .then((result) => {
    ...
    console.log(
      "User not signed in, input is nothing, or input contains <script> tag"
    );
  }
}
...
function initial_message_load(total_messages_queryed) {
  if (auth.currentUser != null) {
    ...
    const query = firebaseFirestore.query(
      ...
      firebaseFirestore.limit(total_messages_queryed)
    ); // query the database ordered by time with a limit of (x)
    firebaseFirestore.getDocs(query).then((snapshot) => {
      // gets the documents the query found and then uses the snapshot to get the data
...
const query = firebaseFirestore.query(
  what_collection,
  firebaseFirestore.orderBy("timestamp", "desc"),
  firebaseFirestore.limit(1)
); // query the database ordered by time with a limit of (x)
const unsubscribe = firebaseFirestore.onSnapshot(query, (snapshot) => {
  // runs anytime anything happens in messages also runs at start for no fucking reason so i had to do a lil work around
...
function generate_divs_from_doc_class(doc_sent, prepend) {
  // gets called every single time a new message gets added to the server, async function
